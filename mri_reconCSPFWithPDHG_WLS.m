...
function out = B( in, op )
  ...
  if doCheckAdjoint
    ...
    [ checkB, errB ] = checkAdjoint( rand( numel( unknownIndxs ), 1 ), @B, ...
      'innerProd', @innerProd );   %#ok<UNRCH>
    if checkB == 0
  ...
  function out = proxG(in, sigma)
    out = proxL1Complex(in, sigma, 1, -b);
  end

  function out = proxGtildestar(in, tau)
    sig = tau/alpha;
    tmp1 = proxgconj(sig*(applyA(in(1:n)) + applyB(in(n+1:end))), sig);
    tmp2 = applyAt(tmp1);
    tmp3 = applyBt(tmp1);
    out = tau * [tmp2; tmp3];
  end
...
%   pgtilde = @(x, t) x - tau * [A'; B'] * pgstar(sigma*(applyA(x(1:n)) + applyB(x(n+1:end))), sigma);
  %%% g conj? idk what we're doing again figure it out
...
%   [xStar4, iters4, alphas] = dr_wLS([k0;zeros(size(k0))], @proxf, @proxGtilde, maxIter);
  ...
  [xStar5, iters5, alphas5] = primal_dual_dr_aoi_wls([k0;zeros(size(k0))], @proxf, @proxGtildestar);
  
  [xStar,objValues,relDiffs,pdhg_iters] = pdhg( k0, @proxf, @proxgconj, tau, ...
