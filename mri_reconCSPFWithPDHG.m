...
function out = mri_reconCSPFWithPDHG( kData, sFSR, varargin )
  ...
  function out = A( in, op )
    if nargin < 2 || strcmp( op, 'notransp' )
      x = zeros( sImg );
      x( unknownIndxs ) = in;
      img = mri_reconPartialFourier( x, sFSR, 'phases', phases );
      out = wtDaubechies2( img, wavSplit );
    else
      WhIn = iwtDaubechies2( in, wavSplit );
      kPF = mri_reconPartialFourier( WhIn, sFSR, 'phases', phases, 'op', 'transp' );
