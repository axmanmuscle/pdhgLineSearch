...
function [xStar,objValues,relDiffs] = pdhg( x, proxf, proxgConj, tau, varargin )
  ...
  while optIter < N
    ...
    bt = (c/(2*tauk))*norm(x - lastX)^2 - 2*(z - lastZ)'*applyA(x - lastX);
    bt = bt + (c/(2*sigmak))*norm(z - lastZ)^2;
    if bt <= 0
      tauk = tauk/2;
      sigmak = sigmak/2;
    end

    pk = (1/tauk) * (lastX - x) - applyAT( lastZ - z );
    dk = (1/sigmak)*(lastZ - z) - applyA(lastX - x);
